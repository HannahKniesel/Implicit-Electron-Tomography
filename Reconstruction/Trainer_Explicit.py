...
from Models.EMGridSimulator import EM_GridSimulator
...
###############################################
# Train Explicit Model for reconstruction     #
###############################################
...
class Trainer_Explicit(Trainer):
    def __init__(self, args, log_folder_path, gt_path):
        args.pos_enc = 0
        args.defocus = False
        super().__init__(args, log_folder_path, gt_path)
        
        # initialize explicit models
        self.simulator_model = EM_GridSimulator(large=True, grid_scale = self.args.grid_res)
        self.simulator_approximation = EM_GridSimulator(large=False, grid_scale = self.args.grid_res//2)
        small_model_params = [self.simulator_approximation.grid]
        model_params = [self.simulator_model.grid] 
        if(self.args.resume_training and self.ckpt): 
            small_grid = self.ckpt['model_small_state_dict']
            self.simulator_approximation.grid = torch.tensor(small_grid, requires_grad=True, device = 'cuda')
            grid = self.ckpt['model_large_state_dict']
            self.simulator_model.grid = torch.tensor(grid, requires_grad=True, device = 'cuda')
    ...
    def training(self):
        ...
        # wandb.watch(self.simulator_model, log_freq=LOG_MODEL)
        ...
        l2_optim_small = torch.optim.Adam([self.simulator_approximation.grid], lr=self.args.lr)
        l2_optim = torch.optim.Adam([self.simulator_model.grid] , lr=self.args.lr)
        ...
        while(True):
            ...
            if((iteration-1) % int(self.args.val_step) == 0):
                ...
                if((psnr > best_psnr) or iteration == self.args.val_step):
                    ...
                    set_param(self.args.grid_res, 'grid_res', dict_saveparams)

                    print("Save Dict: "+str(dict_saveparams))

                    set_param(self.simulator_approximation.grid.detach().cpu().numpy(), 'model_small_state_dict', dict_saveparams)
                    set_param(self.simulator_model.grid.detach().cpu().numpy(), 'model_large_state_dict', dict_saveparams)
    ...
    def predict_slice(self):
        m = self.args.grid_res//2
        recon = min_max(self.simulator_model.grid[:,:,m].squeeze()).detach().cpu().numpy()
        rescale = self.val_shape_vol/recon.shape[0]        
        recon = ndimage.zoom(recon, rescale)
        recon = np.clip(recon, 0, 1)[:,::-1]
        return recon
...
if __name__ == "__main__":
    ...
    parser.add_argument('--grid_res', type = int, default=128, help='Grid resolution to optimize explicit model (default: 128)')
    ...
    t = Trainer_Explicit(args, log_folder_path, gt_path)
    t.training()